...
def check_bio(tags):
    ...
    for i, tag in enumerate(tags):
        ...
        if tag_list[0] == 'B':
            ...
            # 如果第一个位置不是B或者当前编码不是B并且前一个编码O，则全都转换成B
            tags[i] = 'B' + tag[1:]
        elif tags[i - 1][1:] == tag[1:]:
            # 如果当前编码的后面类型编码于tags中的前一个编码中后面类型编码相同
            # 则跳过
            continue
        else:
            # 如果编码类型不一致，则重新从B开始编码
...
def create_dico(item_list):
    """
    对于item_list中每一个item，统计items中的item在item_list中的次数
    item:出现的次数   总之就是每个汉字出现的次数
    :param item_list:
    :return:
    """
...
def get_seg_features(words):
    """
    利用jieba分词
    采用类似BIOES编码，0表示单个成词，1表示开始，2中间，3结尾
    :param words:
    :return:
    """
    seg_features = []
